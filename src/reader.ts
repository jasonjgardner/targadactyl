...
/**
 * Reads and decodes TGA files into top-down RGBA pixel data. Browser-safe
 * and tree-shakable: this module statically imports only types and errors.
 * Node builtins are imported lazily inside `open()` and the `file://`
 * branch of `fetch()`, so bundling for the browser stays clean as long as
 * those filesystem paths are never taken. For canvas/PNG/JPEG output on
 * Node, use `TgaLoader`, which extends this class.
 *
...
export class TgaReader {
  ...
  /**
   * Setter method for TGA header data. Checks the header of TGA file to detect errors before setting `TgaReader._header` property
  ...
  /**
   * Open remote TGA file. `http://` and `https://` URLs use the global
   * Fetch API and work in any runtime, including browsers. `file://` URLs
   * lazily import Node builtins and therefore require Node.js or Bun.
   *
   * @example ```ts
   * const tga = new TgaReader();
  ...
  async fetch(uri: URL): Promise<Uint8ClampedArray> {
    if (uri.protocol === "file:") {
      // Handle file:// URLs by reading from filesystem (Node/Bun only)
  ...
  /**
   * Open local TGA file. `node:fs/promises` is imported lazily, keeping
   * this module browser-safe when `open` is never called.
   *
   * @example ```ts
   * const tga = new TgaReader();
   * const src = "./test.tga";
   ...
   * @returns {this} Returns instance of self used for method chaining
   */
  load(data: Uint8ClampedArray): this {
    let offset = 0;
  ...
  /**
   * Get decoded RGBA pixel bytes for the TGA
   ...
   */
  protected getImageData(): Uint8ClampedArray {
